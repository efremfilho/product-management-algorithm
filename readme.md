...
# pt_br
Para quem é tarado por algoritmo mesmo no mundo do no/low code. Como a gestão de produto funciona.
...
//Para todo novo JTBD, chamar a função - PRODUCT DISCOVERY ou LEARN
aprender(JTBD) {
  enquanto não tiver(débito técnico impactando entrega de valor ou tiver caixa) {
    ...
    se(a solução para o JTBD tiver débito técnico impactando entrega de valor) { //ou seja escalou
      ganhar(débito técnico);
    }
  }
}

//Para todo novo débito técnico - PRODUCT DELIVERY ou EARN
ganhar(débito técnico) {
